...
class RegisterUserScreen extends Component {
    ...
    renderRequestButton() {
        ...
        return (
            <BottomButton title='Cadastrar' onPress={this.checkIfEmailIsValid.bind(this)} />
        );
    }
    ...
    render() {
        ...
        return (
            <View style={mainContainer}>
                <Header title='Cadastrar empregado' onPressLeft={() => goBack()} />
                <ImagePickerModal
                ...
                <ScrollView style={{ marginBottom: 65 }}>
                    <KeyboardAvoidingView behavior="padding">
                        ...
                        <View style={inputContainer}>
                            {this.state.firstName != '' && <FormLabel labelStyle={labelStyle}>Nome</FormLabel>}
                            <FormInput inputStyle={input} placeholderTextColor={Colors.textplaceholder} placeholder='Nome' onChangeText={(text) => this.setState({ firstName: text.trim() })} autoCorrect={false} />
                            {!this.state.firstName && <FormValidationMessage containerStyle={validationMessage}>{this.state.error}</FormValidationMessage>}

                            {this.state.lastName != '' && <FormLabel labelStyle={labelStyle}>Sobrenome</FormLabel>}
                            <FormInput inputStyle={input} placeholderTextColor={Colors.textplaceholder} placeholder='Sobrenome' onChangeText={(text) => this.setState({ lastName: text.trim() })} autoCorrect={false} />
                            ...
                            {this.state.phone != '' && <FormLabel labelStyle={labelStyle}>Telefone</FormLabel>}
                            <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
                                <FormInput inputStyle={input} placeholderTextColor={Colors.textplaceholder} placeholder='Telefone' keyboardType={'numeric'} onChangeText={(text) => this.setState({ phone: Utils.formatPhoneNumber(text) })} value={this.state.phone} />
                            ...
                            {this.state.showJobMessage && <FormValidationMessage containerStyle={validationMessage}>Selecione o cargo</FormValidationMessage>}

                            {this.state.password != '' && <FormLabel labelStyle={labelStyle}>Senha</FormLabel>}
                            <FormInput inputStyle={input} placeholderTextColor={Colors.textplaceholder} placeholder='Senha' onChangeText={(text) => this.setState({ password: text.trim() })} secureTextEntry={true} />
                            {(this.state.passwordInvalid && this.state.password.length < 8) && <FormValidationMessage containerStyle={validationMessage}>No mínimo 8 caracteres</FormValidationMessage>}

                            {this.state.checkPassword != '' && <FormLabel labelStyle={labelStyle}>Confirmar senha</FormLabel>}
                            <FormInput inputStyle={input} placeholderTextColor={Colors.textplaceholder} placeholder='Confirmar senha' onChangeText={(text) => this.setState({ checkPassword: text.trim() })} secureTextEntry={true} />
                            {(this.state.password != this.state.checkPassword) && (this.state.checkPassword.length > 0) && <FormValidationMessage>As senhas devem ser idênticas</FormValidationMessage>}
                        </View>
